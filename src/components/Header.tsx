...
const Header = () => {
  ...
  return (
    <header
      className={`fixed top-0 left-0 w-full flex flex-col gap-2 text-center justify-between py-4 px-10 items-center transition-all duration-500 ease-in-out bg-white mx-auto overflow-hidden z-50 xmd:flex-row shadow-md`}
      style={{ maxHeight: isOpen ? '300px' : '65px' }}
    >
      ...
      <nav>
        {isSignedIn ? (
          <div className="flex flex-col gap-2 xmd:flex-row">
            <Link
              to={'/my-properties'}
              className=" cursor-pointer text-sm p-2 rounded-lg block w-36 bg-blue-600 text-white hover:scale-105 transition duration-300"
            >
              Your listings
            </Link>
            ...
            <Link
              to={'/profile'}
              className="cursor-pointer text-sm p-2 bg-gray-200 text-blue-600 rounded-lg block w-36 hover:scale-105 transition duration-300"
            >
              Profile
            </Link>
          </div>
